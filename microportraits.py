...
def get_name_constituent(deps, tid):
    ...
    my_constituent = [tid]
    for dep in deps:
        if 'mwp' in dep[1] or 'flat' in dep[1]:
...
def analyze_pobject(nafobj, head_id):
    ...
    #sometimes dep has no head (- namely the head/root), make sure not a None-type
...
def investigate_relations_dep(nafobj, tid, term_portrait):
...
def get_activity_relations_from_srl(nafobj, tid, term_portrait):
    '''
    This function retrieves relevant information from the srl dictionary and adds it to the term_portrait object

    :param nafobj: nafobj containing all linguistic annotations
    :param tid: term_identifier
    :param term_portrait: collected information on the term
    NEW
    '''

    global arg2pred

    my_roles = arg2pred.get(tid)
    for role in my_roles:
        pred_id = role[0]
        role_label = role[1]
        add_rows_for_activity_description_srl(pred_id, nafobj, term_portrait, role_label)
...
def get_activity_relations(nafobj, term_portrait):
    ...
    #if rrel_type == 'srl':
    #    investigate_relations_srl(nafobj, tid, term_portrait) #instead of calling the function based on syntax, now call the function that retrieves SRL.

    investigate_relations_dep(nafobj, tid, term_portrait)
...
def add_rows_for_activity_description_srl(head_id, nafobj, term_portrait, stype):
    '''

        :param head_id: identifier of the predicate
        :param nafobj: naf object with all linguistic information
        :param term_portrait: microportrait for the term
        :param stype: semantic role label
        :return:
        NEW
        '''
    ...
    lemma = get_lemma_from_term(nafobj, head_id)
    pos = get_pos_from_term(nafobj, head_id)
    description = cDescription(head_id, lemma, stype, pos)
...
def extract_sentence_portrait(nafobj, term):
    ...
    '''
    global dep2heads, head2deps, arg2pred
    ...
    # extract descriptive information about the entity itself
    if tid in head2deps:
    ...
    #extract relations between the entity and possible activities.
    #   NEW: if any SRL relations are found, then it call the SRL function. Otherwise, it goes back to the dependency based one.
    if len(arg2pred) > 0:
        if tid in arg2pred:
            get_activity_relations_from_srl(nafobj, tid, term_portrait)
    elif tid in dep2heads:
...
def fill_arg2pred_dicts(nafobj):
    '''
    Function that creates dictionary mapping the head of an argument to its predicate with semrole label
    NEW
    '''
    global arg2pred

    naf_srl = nafobj.srl_layer  # self.srl_layer

    # for each predicate, retrieve srl roles
    for pred in naf_srl.get_predicates():
        pred_span = pred.get_span().get_span_ids()
        pred_id = ''
        if len(pred_span) == 1:
            pred_id = pred_span[0]
        else:
            print("WARNING: predicate span has length", len(pred_span))
        for srole in pred.get_roles():
            role_label = srole.get_semRole()
            role_span = srole.get_span().get_span_ids()
            if len(role_span) == 1:
                t_id = role_span[0]
                arg2pred[t_id].append([pred_id, role_label])

def create_info_dicts(nafobj, surface=False):
    '''
    Function that extracts information form naf which we will need a lot
    :param nafobj: input naf
    :return: None
    '''
    ...
    #NEW creating semantic role dictionary
    fill_arg2pred_dicts(nafobj)
...
def extract_sentence_level_portraits(nafobj):
    '''
    Goes through nafobject and extracts microportaits on sentence level
